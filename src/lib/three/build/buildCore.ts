/**
 * Generating a storey's geometry as plain numbers, ready for any thread.
 *
 ...
 * the arithmetic lives here, and what crosses a thread boundary is a set of typed
 * arrays that transfer in constant time rather than a graph of objects that has
 * to be cloned.
 *
 * This module is **pure**: no worker global, no message handler, no side effect
 * on import. That is what lets two different workers share it — the incremental
 * build worker (`build.worker.ts`) and the GLB export worker
 * (`src/lib/export/glb.worker.ts`) both call `buildParts`, and a test imports it
 * to measure the geometry without a thread in the way. The plumbing that claims
 * `onmessage` lives in `build.worker.ts` and only there.
 *
 ...
 *
 * This is the whole compute path of the build worker, and callable without one:
 * a test measures it against the main-thread builders directly, and the GLB
 * export worker calls it for every wall and room it writes out. The message
 * handler in `build.worker.ts` is a thin wrapper round this and nothing else.
 *
