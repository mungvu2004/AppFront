...
export function ConfidenceMeter({ value, className, ...props }: ConfidenceMeterProps) {
  ...
  return (
    <div className={twMerge('flex items-center gap-2', className)} {...props}>
      <div className="relative w-16 h-1 bg-border-default rounded-full overflow-hidden shrink-0">
        <div 
          className={clsx(
      ...
      <span className="font-mono text-[13px] text-text-secondary leading-none w-8">
        {value.toFixed(2)}
      </span>
    </div>
  );
