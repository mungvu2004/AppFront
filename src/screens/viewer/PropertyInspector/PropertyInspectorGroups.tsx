...
function CollapsibleGroup({ group, isFirst }: { group: PropertyGroup; isFirst: boolean }) {
...
export function PropertyInspectorGroups({ groups }: PropertyInspectorGroupsProps) {
  return (
    <div className="flex flex-col">
      {groups.map((group, index) => {
        if (group.id === COLLAPSIBLE_GROUP_ID) {
          return <CollapsibleGroup key={group.id} group={group} isFirst={index === 0} />;
        }
