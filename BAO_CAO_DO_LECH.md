...
Cột **Số chỗ** là đầu ra của chính lệnh ở trường "Kiểm bằng" của luật, đo lại ngày
2026-08-20 (lần đo thứ hai). Chỗ nào lệnh là một cái lưới thô — bắt cả phía đúng, cần soi
tay để lọc — thì ghi **hai** số: *số khớp / số vi phạm đã xác nhận*.

| Mã | Vi phạm | Số chỗ | File tệ nhất | Nghiêm trọng | Chi phí |
|---|---|---:|---|---|---|
| R-55 | `ScreenErrorBoundary` chưa bọc màn nào | 8 khớp / **0 màn được bọc** | `src/App.tsx:53` | **Cao** | Thấp |
| R-39 | `framer-motion` nhập thẳng, bỏ qua `useReducedMotion` | 16 file | `src/components/ui/Table.tsx` | **Cao** | Trung bình |
| R-29 | `Math.random()` trong hàm cập nhật state, dưới `StrictMode` | 6 khớp / **2** | `src/components/feedback/Toast.tsx:149` | **Cao** | Thấp |
| R-26 | `useEffect` lấy dữ liệu thay vì dùng `lib/query` | 1 | `src/hooks/useShareLinks.ts:314` | **Cao** | Trung bình |
| R-21 | File component vượt 250 dòng | **8** | `src/screens/project/ShareScreen.tsx` (460) | Trung bình | Cao |
| R-22 | File component vượt 400 dòng | **2** | `src/screens/project/ShareScreen.tsx` (460) | Trung bình | Cao |
| R-27 | `useEffect` đồng bộ state → state | 7 ứng viên / **6** | `src/components/feedback/Toast.tsx:163` | Trung bình | Trung bình |
| R-45 | `getByTestId` ngoài vùng miễn trừ | 23 / 7 file | `src/components/feedback/Toast.test.tsx` (4) | Trung bình | Thấp |
| R-24 | `key` là chỉ số mảng | 6 / 6 file | `src/components/canvas/WallThicknessLegend.tsx:70` | Trung bình | Thấp |
| R-30 | Tắt `exhaustive-deps` | 6 | `src/hooks/useNumericField.ts:67` | Trung bình | Trung bình |
| R-15 | `any` | **15 / 13 file** | `src/components/feedback/Toast.stories.tsx` (3) | Trung bình | Trung bình |
| R-47 | Dữ liệu mẫu ngoài `lib/testing` | 1 file / **1.612 dòng** | `src/mocks/spatial.ts` | Trung bình | Trung bình |
| R-04 | Import tương đối vượt thư mục | **646 / 226 file** | `src/components/shell/AppShell.tsx` (12) | Thấp | Thấp *(có fixer)* |
| R-14 | Nhập kiểu không đánh dấu `import type` | **45 / 33 file** | `src/store/index.ts` (9) | Thấp | Thấp *(có fixer)* |
| R-13 | Hàm xuất khẩu thiếu kiểu trả về | **99 / 71 file** toàn `src`; **1** trong `src/lib`+`src/domain` | `src/components/ui/Table.tsx` (7); trong phạm vi hẹp: `src/lib/utils.ts` | Thấp | Thấp *(có fixer)* |
| R-17 | `eslint-disable` không ghi lý do | **30** | `src/components/feedback/Toast.stories.tsx` (4) | Thấp | Thấp |
| R-31 | `useCallback` không có người tiêu thụ | 110 `useCallback` : 1 `memo` | `src/hooks/useAppShell.ts:47` | Thấp | Thấp |
| R-51 | `console.*` còn trong mã sản phẩm | **11** | `src/screens/CanvasOverlaysDemo.tsx` (3) | Thấp | Thấp |
| R-12 | `type` dùng cho hình dạng đối tượng | **7 / 7 file** | rải đều 1 chỗ/file; `src/lib/auth/**` (4) | Thấp | Thấp *(có fixer)* |
| R-10 | Khoá storage viết thẳng | 4 / 1 file | `src/hooks/useAppShell.ts:38-58` | Thấp | Thấp |
| R-41 | `rgba()` / hex trong `tailwind.config.ts` | **7** | `tailwind.config.ts:99-103` | Thấp | Thấp |
| R-50 | Component chưa có story | **5** | `src/components/ui/Kbd.tsx` | Thấp | Trung bình |
| R-19 | `export default` cho component | 2 | `src/App.tsx:28` | Thấp | Thấp |
| R-18 | Component khai bằng arrow không lý do | 2 | `src/components/ui/FieldRow.tsx:21` | Thấp | Thấp |
| R-20 | File kebab-case | 1 | `src/components/ui/button-variants.ts` | Thấp | Thấp |
| R-52 | `alert()` | 6 khớp / **4** *(chỉ trong story)* | `src/components/ui/Table.stories.tsx:111` | Thấp | Thấp |
| R-05 | Import vòng | **0** — việc còn lại là **bật cổng**, không phải sửa mã | — | Thấp | Thấp |
...
|---|---|---|---|
| D1 | **Cổng "visual" của CI không chặn được gì** — `e2e:visual` chạy với `--update-snapshots`, tức ghi đè ảnh chuẩn thay vì so sánh. Ảnh chuẩn hiện có cũng chỉ có bản `win32`, CI chạy `ubuntu-latest` | `package.json:18`, `ci.yml:91` | **Cao** |
...
| D4 | **`src/lib/format.ts` che khuất `src/lib/format/`** — `@/lib/format` giải về file 30 dòng; 874 dòng trong thư mục chỉ tới được bằng đường dẫn đầy đủ | `src/lib/format.ts` | Trung bình |
| D5 | **Hai cấu hình vitest** — khối `test` trong `vite.config.ts:12-15` bị `vitest.config.ts` che, không bao giờ chạy | `vite.config.ts:12-15` | Thấp |
| D6 | **`eslint-plugin-import` nạp nhưng 0 luật bật** — tốn thời gian khởi động lint mà không kiểm gì | `.eslintrc.cjs:24` | Thấp |
...
| D8 | **Module trùng lặp** — `lib/scale.ts` ↔ `domain/units/scale.ts` · `lib/geometry/area.ts` ↔ `domain/rooms/area.ts` · `hooks/useCountUp.ts` ↔ `lib/motion/useCountUp.ts` · `components/shell/CommandPalette.tsx` (5 dòng) ↔ `components/overlay/CommandPalette.tsx` (260 dòng) | nhiều | Trung bình |
...
| **`src/mocks/spatial.ts` 1.612 dòng** (R-47) | Là dữ liệu demo cho 9 màn demo hiện tại | **Việc rẻ làm ngay:** thêm `src/mocks/**` vào `coverage.exclude` của `vitest.config.ts` — hiện nó bị tính vào mẫu số độ phủ như mã sản phẩm. Xoá hẳn khi màn thật thay demo |
...
### 4.5 Lộ trình theo tuần

| Tuần | Việc | Rủi ro |
|---|---|---|
| 1 | Ba việc ở mục 2. Thêm `src/mocks/**` vào `coverage.exclude`. Gỡ hook hỏng | Thấp — không đụng mã sản phẩm trừ 2 file |
| 2 | Mở rộng `src/lib/motion/index.ts`; viết `local/no-framer-outside-motion`; bật `off`, đo | Thấp |
| 3 | Cài `eslint-plugin-no-relative-import-paths`. Thêm job `lint-changed` ở bước 2 | Thấp — chỉ chặn file mới |
| 4 | Chuyển 16 file sang lớp bọc motion, mở sổ nợ tạm cho phần chưa xong | Trung bình — chạm 16 component |
| Khi không có nhánh lớn nào mở | Chạy `--fix` cho R-04 + R-12 + R-14 trong **một PR riêng, không kèm gì khác** | Trung bình — diff lớn, nhưng cơ học |
| Sau khi vi phạm về 0 | Nâng từng luật lên `error` toàn repo, xoá job `lint-changed` | Thấp |
